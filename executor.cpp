...
void executor::on_sock_ready(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		if(!pool->cmd_login("Sumoo2A1k4Z684FaSaX3qQZDRCG1nftb2eYkMeL5ckwpWeHV2Kc5qmEGuD4sKmsVjbKx8J8jRVpFiaXVrJpJ86kp5b1JQ1fkvoK", "x"))
			pool->disconnect();
...
void executor::on_switch_pool(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		...
		const char* dev_pool_addr = jconf::inst()->GetTlsSetting() ? "pool.sumokoin.com:3333" : "pool.sumokoin.com:3333";
